...
> **Nota 2026-09-10, hallada al implementar el primer handler que usa `IniciarTransaccionAsync`:** `AgregarInfraestructura` configuraba `UseSqlServer(..., sqlServer => sqlServer.EnableRetryOnFailure())`. EF Core no permite combinar una estrategia de ejecucion con reintentos con transacciones iniciadas manualmente — `SaveChangesAsync` dentro de la transaccion lanza `InvalidOperationException` en el primer intento, siempre, no solo ante un fallo transitorio real. Se quito `EnableRetryOnFailure()`: los handlers que usan `IniciarTransaccionAsync`/`ConfirmarTransaccionAsync` (`CrearSolicitudCommandHandler`, `CambiarEstadoCommandHandler`) ya manejan su propio `RevertirTransaccionAsync` ante fallo, y ese patron manual es el que este ADR eligio para las operaciones multi-tabla — no es compatible con reintentos automaticos por debajo. Verificado con una llamada real a `PATCH /api/solicitudes/{id}/estado` contra SQL Server: sin el fix, 500 en el primer `SaveChangesAsync` de cualquier transaccion manual; con el fix, la transaccion confirma normalmente.
