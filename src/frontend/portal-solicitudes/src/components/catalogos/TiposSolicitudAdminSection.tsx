...
interface TipoSolicitudFormProps {
	editando: TipoSolicitudAdmin | null;
	onGuardar: (valores: TipoSolicitudFormValues) => Promise<void>;
	onCancelar: () => void;
}

function TipoSolicitudForm({
	editando,
	onGuardar,
	onCancelar,
}: TipoSolicitudFormProps) {
	const {
		register,
		handleSubmit,
	...
	return (
		<form
			...
			<FormField
				...
				<textarea
					rows={2}
					placeholder='Detalle del tipo de solicitud'
...
export function TiposSolicitudAdminSection() {
	...
	// Fuerza un remount del formulario tras crear (ver TipoSolicitudForm): cambiar esto
	// cambia su `key`, así que arranca de nuevo con defaultValues vacíos sin necesitar reset().
		...
		try {
			...
			setErrorGeneral(
				error instanceof ErrorApi
					? error.message
					: 'No se pudo guardar el tipo de solicitud.',
			);
		}
	}
